...
from Model.engin import run_one_horizon

# pemsd7 records per 5 min, so horizons = [3,6,9] with n_his = 12 is [15, 30, 45] mins by considering 60 minuts
# airquality records per hour, so horizons = [1,2,3] with n_his = 12 is [1, 2, 3] hours considering 12 hours
# shanghai records per min, so horizons = [5, 10] with n_his = 30 is [5, 10] mins by considering 30 minuts
...
def main(args):
    ...
    DATASET_CONFIGS = {
        "PeMSD7_1026": {"n_his": 12, "horizons": [3, 6, 9], "min_multiplier": 5,  "steps_per_day": 288,  "split_days": [34, 5, 5]},
    ...
    for ds_name, paths in dct_dt.items():
        ...
        dataset_csv_path = args.results_csv / f"metrics_{ds_name}.csv"

        for H in args.horizons:
            for ks in args.ks_values:
                print(f"\nTraining with Ks={ks}")
                result = run_one_horizon(X_raw, A, H, args, device, ds_name, ks=ks)
                ...
                ks_csv_path = args.results_csv / f"metrics_{ds_name}_Ks{ks}.csv"
